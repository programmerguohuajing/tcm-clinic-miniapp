...
export const adminRouter = () => {
  ...
  const optionalStoreId = z.object({ storeId: optionalPositiveInt });
  ...
  app.delete("/admin/users/:id", requireRole("owner"), asyncHandler(async (c) => {
    ...
    } catch (e) { console.error("[delete-user] update appointments failed:", e.message); throw e; }
    try {
      await query("delete from users where id = $1", [id]);
    } catch (e) { console.error("[delete-user] delete user failed:", e.message); throw e; }
